<?php
namespace serjazz\modules\UserManagement\models\rbacDB;

use Exception;
use serjazz\modules\UserManagement\components\AuthHelper;
use Yii;
...
class Role extends AbstractItem
{
    ...
    {
        $userRoles = Yii::$app->session->get(AuthHelper::SESSION_PREFIX_ROLES);
        if(Yii::$app->user->isCompany && ($companyKey = array_search('company',$userRoles)) !== false){
            unset($userRoles[$companyKey]);
        }
        if(Yii::$app->user->isManager && ($companyKey = array_search('manager',$userRoles)) !== false){
            unset($userRoles[$companyKey]);
        }
        $condition = (Yii::$app->user->isSuperAdmin OR $showAll) ? [] : ['name'=>$userRoles];

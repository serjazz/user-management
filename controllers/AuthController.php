...
use serjazz\modules\UserManagement\components\BaseController;
                ...
                if ( $this->triggerModuleEvent(UserAuthEvent::AFTER_REGISTRATION, ['model'=>$model, 'user'=>$user]) )
                {
                    if ( $user )
                    {
                        ...
                        else
                        {
                            ...
                            if(Yii::$app->getModule('user-management')->needModeration){
                                return $this->renderIsAjax('confirmEmailSuccessWaitModeration', compact('user'));
                            } else {
